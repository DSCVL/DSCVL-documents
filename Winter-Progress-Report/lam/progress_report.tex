...
\begin{singlespace}
	...
	\section{Current State}
		...
		\subsubsection{Proposed Design}
			...
			The LEDDAR M16 provides accurate depth measurement in a horizontal dimension but lacks vertical depth sensing beyond a 40-degree spread.
		...
		\subsection{Computer Vision}
			Over the past term, my contribution towards our proposed design was to create a reliable image-recognition model.
			I first started with OpenCV's pre-trained facial/pedestrian support-vector-machine (SVM) classifier.
			This SVM is a combination of several other SVMs that detect facial features.
			The completed SVM is intended to detect faces with high accuracy.
			However, when applied to our design, I could not consistently replicate good results.
			This was due to several factors, namely the SVM used is meant to perform classification on still images where the camera's perspective is far from the subject.
			...
			The Tensorflow object recognition library is better suited for this project because its actively maintained library has already been trained to recognize a large dataset of objects.


			\begin{figure}[here]
			\includegraphics[scale=0.5]{tensorflow.PNG}
			\captionof{figure}{Tensorflow pre-trained model can detect multiple subjects.}
			\label{tf-detect}
			\end{figure}
